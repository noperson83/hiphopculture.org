...
class AudioPlaylist{
    ...
    setTrack(arrayPos){
        var liPos = trackOrder[arrayPos]; // convert array index to html index
        var staticUrl = $("#" + playlistId + " li a").eq(liPos).data("data-static-url"); // Get the data attribute for the static file URL
        player.src = staticUrl;
        $("." + currentClass).removeClass(currentClass);
        $("#" + playlistId + " li").eq(liPos).addClass(currentClass);
